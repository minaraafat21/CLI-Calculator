...
// wrapper functions takes args passed to python, process them in C, and return
// the result back to Python.
static PyObject *py_add(PyObject *self, PyObject *args) {
  double a, b;
  if (!PyArg_ParseTuple(args, "dd", &a,
                        &b))  // parses the arguments passed to a Python C
                              // function into local C variables.
    return NULL;  // If the parsing fails, it returns NULL, which indicates an
...
static PyObject *py_power(PyObject *self, PyObject *args) {
  double a, b;
  if (!PyArg_ParseTuple(args, "dd", &a, &b)) return NULL;
  return Py_BuildValue("d", power(a, b));
}

// method table
static PyMethodDef CalculatorMethods[] = {
    // {callable name, C function pointer, METH_VARARGS-> creates a tuple of
    // args passed to function , docstring}
    {"add", py_add, METH_VARARGS,
     "Add two numbers"},  // ex: calculator.add(1, 2)
    ...
    {"power", py_power, METH_VARARGS, "Power"},
